...
class AWSLocationHelpersTests(APITestCase):
    ...
    def test_aws_place_autocomplete_success(self, mock_get_client):
        ...
        self.assertEqual(
            results[0]["structured_formatting"]["main_text"], "15a Kunle Ogunba St"
        )
        self.assertEqual(
    ...
    def test_aws_place_details_success(self, mock_get_client):
        ...
        self.assertEqual(
            details["formatted_address"], "15a Kunle Ogunba St, Lekki, Lagos, NGA"
        )
    ...
    def test_aws_reverse_geocode_success(self, mock_get_client):
        ...
        mock_client.search_place_index_for_position.return_value = {
            "Results": [
                {"Place": {"Label": "Lekki Peninsula, Victoria Island, Lagos, NGA"}}
            ]
        }
        mock_get_client.return_value = mock_client
...
class GeoapifyTests(APITestCase):
    ...
    def test_geoapify_geocode_address(self, mock_get):
        mock_response = MagicMock()
        mock_response.raise_for_status.return_value = None
        mock_response.json.return_value = {"results": [{"lat": 6.43, "lon": 3.48}]}
        mock_get.return_value = mock_response
    ...
    def test_geoapify_reverse_geocode(self, mock_get):
        ...
        mock_response.raise_for_status.return_value = None
        mock_response.json.return_value = {
            "results": [{"formatted": "Lekki Phase 1, Lagos, Nigeria"}]
        }
        mock_get.return_value = mock_response
...
class MapboxTests(APITestCase):
    ...
    def test_mapbox_place_autocomplete_success(self, mock_get):
            ...
            "features": [
                {
                    "type": "Feature",
                    "id": "mapbox_123",
                    "geometry": {"type": "Point", "coordinates": [3.456, 6.45]},
                    "properties": {
        ...
        self.assertEqual(
            res[0]["description"], "15a Kunle Ogunba St, Lekki, Lagos, Nigeria"
        )
        self.assertEqual(
            res[0]["structured_formatting"]["main_text"], "Kunle Ogunba St"
        )
        self.assertEqual(
            res[0]["structured_formatting"]["secondary_text"], "Lekki, Lagos, Nigeria"
        )
    ...
    def test_mapbox_place_details_success(self, mock_get):
        ...
        self.assertIsNotNone(res)
        self.assertEqual(
            res["formatted_address"], "15a Kunle Ogunba St, Lekki, Lagos, Nigeria"
        )
...
@override_settings(
    GEOAPIFY_API_KEY="test-geoapify-key", MAPBOX_ACCESS_TOKEN="test-mapbox-token"
)
