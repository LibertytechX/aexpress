    ...
    def handle(self, *args, **kwargs):
        ...
        vehicles_data = [
            {
                'name': 'Bike',
                'max_weight_kg': 10,
                'base_price': 1200.00,  # Legacy field (deprecated)
            ...
            },
            {
                'name': 'Car',
                'max_weight_kg': 70,
                'base_price': 4500.00,   # Legacy field (deprecated)
            ...
            },
            {
                'name': 'Van',
                'max_weight_kg': 600,
                'base_price': 12000.00,  # Legacy field (deprecated)
        ...
        for vehicle_data in vehicles_data:
            ...
            if created:
                created_count += 1
                self.stdout.write(
                    self.style.SUCCESS(f'✓ Created vehicle: {vehicle.name} - Base: ₦{vehicle.base_fare} + ₦{vehicle.rate_per_km}/km + ₦{vehicle.rate_per_minute}/min')
                )
            else:
                updated_count += 1
                self.stdout.write(
                    self.style.WARNING(f'↻ Updated vehicle: {vehicle.name} - Base: ₦{vehicle.base_fare} + ₦{vehicle.rate_per_km}/km + ₦{vehicle.rate_per_minute}/min')
                )
