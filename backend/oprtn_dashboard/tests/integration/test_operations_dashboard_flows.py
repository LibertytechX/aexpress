...
from oprtn_dashboard.metrics import cod_metrics, payment_metrics
from oprtn_dashboard.models import Alert, AlertRule, AlertStatus, AlertType, FuelBill
...
class OpsTestBase(TestCase):
    ...
    def make_vehicle(self, name="TBike"):
        return Vehicle.objects.create(name=name, max_weight_kg=20, base_price=0)
    ...
    def make_order(
        ...
        return Order.objects.create(
            ...
            cod_amount=Decimal(str(cod_amt)),
        )
...
class Phase4EngineTests(OpsTestBase):
    ...
    def test_gps_offline_fires(self):
        ...
        self.assertTrue(Alert.objects.filter(alert_type=AlertType.GPS_OFFLINE).exists())
...
class AlertEndpointTests(OpsTestBase):
    def _make_alert(self, **kw):
        defaults = dict(
            ...
            severity="high",
            title="t",
            dedupe_key=secrets.token_hex(6),
        )
        defaults.update(kw)
    ...
    def test_alert_dashboard(self):
        ...
        data = self.client.get("/api/ops/alert-dashboard/?period=this_month").json()[
            "data"
        ]
...
class PaymentCodTests(OpsTestBase):
    ...
    def test_payment_orders_drilldown(self):
        ...
        rc = self.client.get(base + "&collection_timing=rider_collected").json()["data"]
        self.assertEqual(rc["count"], 1)  # the cash order
        deferred = self.client.get(base + "&collection_timing=deferred").json()["data"]
...
class FuelTests(OpsTestBase):
    ...
    def _upload(self, rows, client=None, **make_kwargs):
        ...
        return client.post("/api/ops/fuel/upload/", {"file": bio}, format="multipart")
    ...
    def test_upload_rejects_non_xlsx(self):
        ...
        bad.name = "data.txt"
        r = self.client.post("/api/ops/fuel/upload/", {"file": bad}, format="multipart")
...
class FilterTests(TestCase):
    def test_single_date(self):
        s, e, desc = parse_filter(_StubRequest(filter="single_date", date="2025-03-15"))
...
class CacheRefreshTests(OpsTestBase):
    def test_refresh_warms_cache_and_endpoint_serves_it(self):
        ...
        from oprtn_dashboard.caching import get_cached
        from oprtn_dashboard.tasks import refresh_dashboard_cache
...
class OrderDashboardTests(OpsTestBase):
    def test_order_dashboard_levels(self):
        ...
        data = self.client.get("/api/ops/order-dashboard/?period=this_month").json()[
            "data"
        ]
        mg = data["management"]
    ...
    def test_order_orders_drilldown(self):
        ...
        by_rider = self.client.get(base + f"&rider={rider.rider_id}").json()["data"]
...
class TrackingDashboardTests(OpsTestBase):
    def test_tracking_snapshot_and_linkage(self):
        ...
        Rider.objects.create(
            user=self.make_user(contact="Mover"),
            vehicle_asset=a1,
            status="on_delivery",
            is_moving=True,
            current_latitude=Decimal("6.45"),
            current_longitude=Decimal("3.39"),
            current_speed=Decimal("20"),
        ...
        a2 = self.make_asset(
            ...
            speed=0,
            engine_status="off",
            last_telemetry_at=now - timedelta(hours=5),
        )
        Rider.objects.create(
            user=self.make_user(contact="Resting"),
            vehicle_asset=a2,
            status="offline",
            is_moving=False,
            current_latitude=Decimal("6.46"),
            current_longitude=Decimal("3.40"),
        ...
        plates = [it["vehicle"]["plate_number"] for it in data["live"] if it["vehicle"]]
...
class RiderBehaviorTests(OpsTestBase):
    ...
    def done_order(self, rider, amount, distance=None, picked=None, completed=None):
        order = self.make_order(self.merchant, self.vehicle, amount=amount, rider=rider)
        Order.objects.filter(pk=order.pk).update(
    ...
    def test_overriding_dashboard_endpoint(self):
        ...
        self.assertEqual(data["top_riders"][0]["total_overriding_km"], 12.0)
    ...
    def test_attendance_report_online_offline_split(self):
        ...
        row = next(r for r in report["riders"] if r["rider_id"] == self.rider.rider_id)
    ...
    def test_attendance_dashboard_endpoint(self):
        ...
        self.assertEqual(data["top_offline_moving"][0]["offline_moving_km"], 7.0)
    ...
    def test_revenue_leaderboard_top_and_bottom(self):
        ...
        self.assertEqual(bottoms[self.rider_c.rider_id]["net_revenue"], "0.00")
        self.assertEqual(bottoms[self.rider_b.rider_id]["net_revenue"], "800.00")

    def test_low_revenue_alert_only_for_working_riders(self):
        self._seed_leaderboard()
        run_all_rules(only_types=[AlertType.LOW_REVENUE])
        fired = set(
            Alert.objects.filter(alert_type=AlertType.LOW_REVENUE).values_list(
                "rider__rider_id", flat=True
            )
        )
    ...
    def test_order_leaderboard_and_low_volume_alert(self):
        ...
        fired = set(
            Alert.objects.filter(alert_type=AlertType.LOW_ORDER_VOLUME).values_list(
                "rider__rider_id", flat=True
            )
        )
    ...
    def test_leaderboard_endpoints(self):
        ...
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["data"]["top_riders"][0]["net_revenue"], "12000.00")
    ...
    def test_fuel_misuse_dashboard_endpoint(self):
        ...
        self.assertEqual(data["flagged"][0]["rider_id"], self.rider.rider_id)
