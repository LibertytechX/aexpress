...
function ForgotPasswordScreen({ onBack }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const data = await window.API.Auth.requestPasswordReset(email);
...
function ResetPasswordScreen({ token, onComplete }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const data = await window.API.Auth.resetPassword(token, newPassword);
...
function VerifyEmailScreen({ token, onComplete }) {
  ...
  const verifyEmail = async () => {
    try {
      const data = await window.API.Auth.verifyEmail(token);
