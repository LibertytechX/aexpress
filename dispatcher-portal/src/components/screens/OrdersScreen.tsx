    ...
    return (
        <div>
            ...
            <div style={{ background: S.card, borderRadius: 14, border: `1px solid ${S.border}`, overflow: "hidden" }}>
                <div style={{ display: "grid", gridTemplateColumns: "100px 90px 1fr 1fr 1fr 1fr 100px 70px 70px", padding: "10px 16px", background: S.borderLight, fontSize: 10, fontWeight: 700, color: S.textMuted, textTransform: "uppercase", letterSpacing: "0.5px", borderBottom: `1px solid ${S.border}` }}>
                    <span>ID</span><span>Date</span><span>Customer</span><span>Merchant</span><span>Route</span><span>Rider</span><span>Amount</span><span>COD</span><span>Status</span>
                </div>
                <div style={{ maxHeight: "calc(100vh - 310px)", overflowY: "auto" }}>
                    {filtered.map(o => {
                        // Basic format, try to parse created timestamp or use created_at
                        const dateStr = o.created || (o as any).created_at;
                        let displayDate = "";
                        let displayTime = "";
                        if (dateStr) {
                            try {
                        ...
                        return (
                            <div key={o.id} onClick={() => onSelect(o.id)} style={{ display: "grid", gridTemplateColumns: "100px 90px 1fr 1fr 1fr 1fr 100px 70px 70px", padding: "12px 16px", borderBottom: `1px solid ${S.borderLight}`, cursor: "pointer", alignItems: "center", transition: "background 0.12s" }} onMouseEnter={e => e.currentTarget.style.background = S.borderLight} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                                <span style={{ fontSize: 11, fontWeight: 700, color: S.gold, fontFamily: "'Space Mono',monospace" }}>{o.id}</span>
                                <div>
                                    <div style={{ fontSize: 11, fontWeight: 600, color: S.navy }}>{displayDate || "—"}</div>
                                    <div style={{ fontSize: 10, color: S.textMuted, fontFamily: "'Space Mono',monospace" }}>{displayTime}</div>
                                </div>
