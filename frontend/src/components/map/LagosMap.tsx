...
export function LagosMap({ orders, riders, highlightOrder, small, showZones, relayNodes, zones, mode }: any) {
    ...
    const mockPins = [
        { id: "AX-6158260", px: 36, py: 40, dx: 55, dy: 52, label: "Yaba→VI", color: S.gold, status: "Started", rider: "Musa K." },
        { id: "AX-6158261", px: 38, py: 48, dx: 56, dy: 56, label: "Surulere→VI", color: "#a855f7", status: "PickedUp", rider: "Chinedu O." },
        { id: "AX-6158262", px: 30, py: 28, dx: 78, dy: 50, label: "Ikeja→Lekki", color: "#eab308", status: "Pending", rider: null },
        { id: "AX-6158263", px: 26, py: 42, dx: 34, dy: 30, label: "Mushin→Ikeja", color: "#3b82f6", status: "Assigned", rider: "Kola A." },
        { id: "AX-6158258", px: 55, py: 50, dx: 72, dy: 50, label: "VI→Lekki Ph1", color: S.gold, status: "Assigned", rider: "Ahmed B." },
        { id: "AX-6158257", px: 36, py: 40, dx: 54, dy: 52, label: "Yaba→VI", color: S.green, status: "Done", rider: "Musa K." },
        { id: "AX-6158255", px: 72, py: 48, dx: 76, dy: 52, label: "Lekki→Lekki", color: S.green, status: "Done", rider: "Emeka N." },
    ];
    const mockRiderDots = [
    ...
    const pins = isRelayMode ? [] : (orders && orders.length > 0 ? orders.map((o: any, i: number) => {
        // Fallback to generating mock visual positions for real orders if we don't have lat/lng
        // For dashboard purposes and aesthetic demonstration
        const ref = mockPins[i % mockPins.length];
        return {
            ...ref,
            id: o.id,
            label: `${o.pickup?.substring(0, 6) || ''}→${o.dropoff?.substring(0, 6) || ''}`,
            status: o.status
        }
    }) : mockPins);
    ...
    const activeOrders = pins.filter((p: any) => !["Done", "CustomerCanceled", "DriverCanceled", "SupportCanceled", "Failed"].includes(p.status));
    const displayPins = highlightOrder ? pins.filter((p: any) => p.id === highlightOrder) : (mapView === "live" ? activeOrders : pins);

    return (
        <div style={{ position: "relative", width: "100%", height: h, borderRadius: 14, overflow: "hidden", border: `1px solid #e2e8f0`, background: "#EEF2F7" }}>
            ...
            {(mapView === "zones" || showZones) && !isRelayMode && zonesToRender.map((z: any) => (
                <div key={z.id} style={{ position: "absolute", left: `${z.x}%`, top: `${z.y}%`, width: `${z.w}%`, height: `${z.h}%`, background: z.color, border: "1px dashed rgba(0,0,0,0.12)", borderRadius: 8, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: 1 }}>
                    ...
                    <div style={{ fontSize: 6, color: S.grayLight, opacity: 0.6, textAlign: "center" }}>{z.areas}</div>
                </div>
            ))}
            ...
            {!isRelayMode && (
                <svg width="100%" height="100%" style={{ position: "absolute", top: 0, left: 0, pointerEvents: "none", zIndex: 4 }} viewBox="0 0 100 100" preserveAspectRatio="none">
                    {displayPins.filter((p: any) => !["Done", "CustomerCanceled", "DriverCanceled", "SupportCanceled", "Failed"].includes(p.status)).map((p: any) => (
                        <g key={p.id + "route"}>
            ...
            {!isRelayMode && displayPins.map((p: any) => {
                ...
                const cx = p.status === "Done" ? p.dx : p.status === "Pending" ? p.px : (p.px + p.dx) / 2;
                const cy = p.status === "Done" ? p.dy : p.status === "Pending" ? p.py : (p.py + p.dy) / 2;
                return (
                    <div key={p.id} onMouseEnter={() => setHoverPin(p.id)} onMouseLeave={() => setHoverPin(null)}
                        ...
                        {isH && (
                            <div style={{ position: "absolute", top: -40, left: "50%", transform: "translateX(-50%)", background: "#fff", padding: "4px 8px", borderRadius: 6, boxShadow: "0 2px 12px rgba(0,0,0,0.15)", whiteSpace: "nowrap", zIndex: 20, border: `1px solid #e2e8f0` }}>
                                ...
                                {p.rider && <div style={{ fontSize: 7, color: S.grayLight }}>🏍️ {p.rider}</div>}
                            </div>
                        )}
                ...
                <div style={{ position: "absolute", top: 8, right: 8, display: "flex", gap: 4, zIndex: 20 }}>
                    {[{ id: "live", label: "Live", icon: "📡" }, { id: "zones", label: "Zones", icon: "🗺️" }, { id: "heatmap", label: "Heat", icon: "🔥" }].map(v => (
                        <button key={v.id} onClick={() => setMapView(v.id)} style={{
                            padding: "4px 8px", borderRadius: 6, border: `1px solid ${mapView === v.id ? S.gold : "#e2e8f0"}`,
                            background: mapView === v.id ? "rgba(232,168,56,0.12)" : "rgba(255,255,255,0.95)",
                            cursor: "pointer", fontSize: 8, fontWeight: 700, color: mapView === v.id ? S.gold : S.grayLight,
            ...
            <div style={{ position: "absolute", bottom: 8, right: 8, display: "flex", gap: 8, background: "rgba(255,255,255,0.95)", padding: "5px 10px", borderRadius: 8, boxShadow: "0 1px 4px rgba(0,0,0,0.08)", zIndex: 10 }}>
                {!isRelayMode && <>
                    <span style={{ fontSize: 8, color: S.grayLight, display: "flex", alignItems: "center", gap: 3 }}><span style={{ width: 6, height: 6, borderRadius: "50%", background: S.gold }} /> Active</span>
                    <span style={{ fontSize: 8, color: S.grayLight, display: "flex", alignItems: "center", gap: 3 }}><span style={{ width: 6, height: 6, borderRadius: "50%", background: S.green }} /> Rider</span>
                    <span style={{ fontSize: 8, color: S.grayLight, display: "flex", alignItems: "center", gap: 3 }}><span style={{ width: 6, height: 6, borderRadius: "50% 50% 50% 0", transform: "rotate(-45deg)", background: "#eab308" }} /> Pending</span>
                    <span style={{ fontSize: 8, color: S.grayLight, display: "flex", alignItems: "center", gap: 3 }}><span style={{ width: 6, height: 6, borderRadius: "50% 50% 50% 0", transform: "rotate(-45deg)", background: "#a855f7" }} /> In Progress</span>
                </>}
                {isRelayMode && zonesToRender.length > 0 && <span style={{ fontSize: 8, color: S.grayLight, display: "flex", alignItems: "center", gap: 3 }}><span style={{ width: 10, height: 6, borderRadius: 6, border: "1px dashed rgba(59,130,246,0.55)", background: "rgba(59,130,246,0.10)" }} /> Zone</span>}
                {relayNodes && relayNodes.length > 0 && <span style={{ fontSize: 8, color: S.grayLight, display: "flex", alignItems: "center", gap: 3 }}><span style={{ width: 6, height: 6, borderRadius: "50%", background: "#8B5CF6" }} /> Relay Hub</span>}
            </div>

            {/* Live stats overlay */}
            {!isRelayMode && !small && mapView === "live" && (
                <div style={{ position: "absolute", bottom: 8, left: 8, background: "rgba(15,23,42,0.9)", padding: "6px 12px", borderRadius: 8, zIndex: 10 }}>
